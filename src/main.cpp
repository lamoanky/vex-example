...
void opcontrol() {
  ...
  while (true) {
    ...
    // chassis.arcadeq_flipped(ez::SPLIT); // Flipped split arcade
    ...
    // . . .
    handle_input();
    if (ctr.get_digital_new_press(pros::E_CONTROLLER_DIGITAL_R1)) {
        pneum_intake_direction = !pneum_intake_direction;
        pneum_intake.set_value(pneum_intake_direction);
    }
